...
class AgentKernel @Inject constructor(
    ...
    private suspend fun runPipeline(
        ...
        // ── TẦNG 2A: MÀNG LỌC PHỦ ĐỊNH CỤC BỘ (SỬ DỤNG LOOKBEHIND UNICODE LETTER SỬA BUG 3) ──
        ...
        val hasNegation = negationWords.any { word -> 
            Regex("(?<!\\p{L})$word(?!\\p{L})").containsMatchIn(lowerMsg) // SỬA BUG 3: Unicode boundaries
        }
                ...
                is Layer3Result.Multi -> {
                    ...
                    layer4Result.intents.forEach { (plugin, intent) ->
                        try {
                            val r = executeIntent(plugin, intent, context, traceId)
                            val msg = when (r) {
                                // SỬA BUG cú pháp: đổi từ dấu hai chấm ':' thành 'is'
                                is PluginResult.Success -> "✅ ${intent.pluginId}.${intent.action} thành công."
                                is PluginResult.Failure -> "❌ ${intent.pluginId}.${intent.action} thất bại: ${r.error}" // SỬA BUG tham chiếu biến: đổi từ executionResult.error thành r.error
                                is PluginResult.NeedMoreInfo -> "⚠️ ${intent.pluginId}.${intent.action} thiếu: ${r.question}" // SỬA BUG tham chiếu biến: đổi từ executionResult.question thành r.question
                            }
                            results.add(msg)
                        } catch (e: Exception) {
                            results.add("❌ ${intent.pluginId}.${intent.action}: ${e.message}")
    ...
    private suspend fun tryTier2SemanticSlotResolver(
        ...
        // SỬA BUG 4: Sử dụng phân tích cú pháp JSONObject thay cho kiểm tra contains chuỗi cứng
